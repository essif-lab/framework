...
| `termselcrit` | Y | List of [term selection instructions](@) that are used to generate (this version of) the [scope's](@) [terminology](@). See [Terminology Construction](terminology-construction) for details. |
...
:::info Editor's note
The `from` and `to` dates have been included to (in future) enable one to refer to a specific version of the terminology that was valid at a particular date. This feature needs to be worked out, and will impact [terminology construction](terminology-construction), [term ref specs](tev2-spec-term-ref), and various tools.
:::
