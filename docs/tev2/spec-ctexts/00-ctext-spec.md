...
| ------------ | :---: | ------- | ----------- |
| `id`           | Y*| | Text that [identifies](@) this [curated text](@) within the context of the [scope](@). The value of this field can be used as the `id`-field of a [term ref](@) that refers to the [knowledge artifact](@) described by this [curated text](@). It is RECOMMENDED that its value is constructed such that [authors](@) would use it in a natural way, e.g. by making it equal to `termid` or `<termtype>-<termid>`.<br/>Must satisfy regex `[a-z0-9_-]+`. |
