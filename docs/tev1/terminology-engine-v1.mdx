...
- `term-name`: The filename of the term file, which resides under `/docs/terms/` directory.
  Example: `[actions](@)` will highlight `actions` for the reader, and refer it to the page `/docs/terms/action.md`
...
- `glossaryText` (optional) is a string that is put in the glossary as the description of the term. If `%%`-syntax is used in a `glossaryText` string, **the `|` character must be replaced with the `^`-character**. Example: `[actors](@)` is NOT allowed in the `glossaryText`. It should be `%%actors^actor%%`.
