...
There are various ways for [parties](@) to come to grips with specifying and [managing](management@) (or [governing](governance@)) their [objectives](@). Here are some examples:
...
- [Organizations](@) are also known to create [objectives](@) for (primary and/or secondary) business processes; such [objectives](@) can be grouped around a process, or cluster of related processes, for efficient and effective [management](@). Examples include [compliance objectives](compliance-objective@) (i.e. [objectives](@) of a [compliance](@) process), and [risk objectives](risk-objective@) (i.e. [objectives](@) of a [risk management](@) process).
- [Organizations](@) may classify their [objectives](@) using the [formalization of objectives](./objective#formalization) (and the [governance and management pattern](pattern-governance-and-management@)), e.g.
  - [expectations](@), i.e. [objectives](@) the result of which is consumed, but not produced by the [organization](@). Such [objectives](@) are not [managed](management@) but [governed](governance@), and hence require communications with the [parties](@) that produce the expected results;
  - [obligations](@), i.e. [objectives](@) the result of which is produced by the [organization](@) and consumed by (at least) one other [party](@). Such [objectives](@) are [managed](management@), and also require communication with the consuming [parties](@), e.g. in terms of advertising the results so that consuming [parties](@) will request the produced results, or in terms of responding to [parties](@) that request to produce certain results;
  - controls, i.e. [objectives](@) the result of which is produced and (only) consumed by the [organization](@) itself. These [objectives](@) must be [managed](management@) and [governed](governance@) by the [party](@) that [owns](@) them, and any 'communication' would be between [actors](@) of that [party](@).
...
[Parties](@) that [owns](@) [objectives](@) need to distinguish between the following kinds:

1. [expectations](@), because such kinds of [objectives](@) can only be [governed](governance@) as [managing](management@) them (i.e. the production of the associated results) is out of their [scope of control](@).
2. [obligations](@), because such kinds of [objectives](@) must be [managed](management@) to take requirements into account that make them fit for the purpose that the [parties](@) that consume such results want them to serve. This requires communication with such [parties](@) to obtain the 'fit-for-purpose criteria' that the results need to satisfy.
3. [control objectives](control-objective@), because these kinds of [objectives](@) are to be [governed](governance@) as well as [managed](management@). The process for [managing](management@) the production and [governing](governance@) the consumption of the associated results are best integrated into a [risk management](@) process.
