...
-   when it is instructed by, or on behalf of its Owner, to request a specific kind of transaction to some Agent of another %%party|party%%.[^one]
...
-   for each such transaction type:
    ...
    -   the kinds of credentials and issuers that the Owner is willing to accept as sources for valid data; (optionally?), the endpoint URI at which issuing %%parties|party%% do the actual credential issuing may be specified[^6].
