...
One of the fundamental ideas in this pattern is that our %%party|party%% concept is not considered to (be able to) act. We postulate that a %%party|party%% requires %%actors|actor%% (i.e. %%entities|entity%% that _can_ act) to act on their behalf. We can nevertheless continue to use the commonly used form of speech in which a %%party|party%% is said to perform some %%action|action%%, by realizing that this is to be interpreted as that there is (at least) one %%actor|actor%% that is actually executing that %%action|action%% on behalf of this %%party|party%%. For example: an %%organization|organization%% doesn't sign a contract; rather, this is done by some %%actor|actor%% (e.g. some officer of the %%organization|organization%%) that does so on the %%organization's|organization%% behalf.

Postulating that a %%party|party%% cannot act does, however, not preclude the existence of %%entities|entity%% that are both %%party|party%% and %%actor|actor%%. %%Humans|human-being%% are the canonical example of this: a %%human|human-being%% (as an %%actor|actor%%) can act on its %%own|owner%% behalf (as a %%party|party%%). However, like other %%actors|actor%%, it can also act on behalf of other %%parties|party%%, which we will get to further down.

### Knowledge, Objectives and Policies
...
%%Knowledge|knowledge%% also contains %%objectives|objective%% that its %%owner|owner%% seeks to be realized and triggers all sorts of %%actions|action%% to be performed. To ensure that such %%actions|action%% are properly executed (by %%agents|agent%%, that may well not be the %party itself), rules, working instructions and/or other guidance for the execution of such %%actions|action%% (i.e.: %%policies|policy%%) must exist. Such %%policies|policy%% might specify, e.g., when %%actions|action%% should start or terminate, which (kinds of) %%actors|actor%% would qualify for executing it, etc.

This works well for %%human beings|human-being%%, which are both a %%party|party%% and an %%actor|actor%%. So a %%human being|human-being%% can act, implying itself as an %%actor|actor%%, and using its personal %%knowledge|knowledge%% as guidance. As we shall see, the model also works when a %%human being|human-being%% (as a %%party|party%%) may hire someone else (as an %%actor|actor%%), e.g. to fill in his tax return form. This other is guided by the %%knowledge|knowledge%% of the %%human being|human-being%% that hired him, and uses its own %%knowledge|knowledge%% for the details of filling in the tax form.
...
:::note
Note that an %%actor|actor%% that executes an %%action|action%% on behalf of some %%party|party%% can also be said to 'represent' that %%party|party%%. Note that it is important to distinguish between *actual representation* of a %%party|party%% (as described here) and *potential representation*, as explained in the section [Potential representation of parties](#potential-representation-of-parties).
:::
...
The relevance of the %%agent|agent%%-%%principal|principal%% relationship is that not only must there be at least one %%action|action%% that the %%agent|agent%% is in the process of executing on behalf of the %%principal|principal%%, but also there must be a %%policy|policy%% that is part of its %%principal's|principal%% %%knowledge|knowledge%%, and that provides the %%agent|agent%% with the rules, working instructions and/or other guidance for the execution of that %%action|action%%.

This %%policy|policy%% (of the %%principal|principal%%) may provide a limited guidance, particularly in situation where the %%agent|agent%% can be assumed to have access to other %%knowledge|knowledge%%. For example, a robot can do things with limited instructions, as the detailed instructions (e.g., for moving, rotating, etc.) would come from an internal program (that represents %%knowledge|knowledge%% of others about how to move, rotate, etc.). Also, %%humans|human-being%% only need limited guidance when they execute an %%action|action%% for some other %%party|party%%, because they have their own %%knowledge|knowledge%% that is typically relied on for the details of such execution. For example, a %%human|human-being%% that is tasked by the NGO it works for to register refugees will need guidance regarding the registration process itself and the data to register, and it will use it's own %%knowledge|knowledge%% to work the computer (e.g., typing, operating the mouse).

### Working for a party: onboarding

This idea of %%actors|actor%% and %%parties|party%% that can engage in an %%agent|agent%%-%%principal|principal%% relationship, where the %%agents|agent%% accesses and uses a %%policy|policy%% (of its %%principal|principal%%) for executing a specific %%action|action%%, implies that %%parties|party%% need to onboard the %%actors|actor%% before they can enter into an %%agent|agent%%-%%principal|principal%% relationship, where '%%onboarding|onboarding%% of an %%actor|actor%% (by a %%party|party%%)' means that the %%party|party%% consecutively do the following:
- First, the %%party|party%% **establishes the suitability** of the %%actor|actor%% for executing %%actions|action%% of certain types on its behalf, e.g. in terms of its capabilities, capacities, characteristics etc. This means that the %%actor|actor%%, once it is %%onboarded|onboarding%%, is capable of working as the %%party|party%% expects it to.
- Second, the %%party|party%% **ensures that duties exist** for the %%actor|actor%% to actually execute %%actions|action%% of such types according to the appropriate %%policies|policy%% of that %%party|party%%. This means that the %%actor|actor%%, once it is %%onboarded|onboarding%%, can actually be relied upon to work as the %%party|party%% expects it to.
- Third, the %%party|party%% **provides all circumstances and/or satisfies all conditions** that the %%actor|actor%% needs to execute %%actions|action%% of designated types according to that %%party's|party%% %%policies|policy%%. This includes e.g. providing the %%actor|actor%% with roles/permissions, resources to work with, etc.

We use the relation `works for` to model the fact that an %%actor|actor%% has been %%onboarded|onboarding%% by a %%party|party%%. Equivalently, we can say that an %%actor|actor%% and a %%party|party%% are in an %%employee|employee%%-%%employer|employer%% relationship. Thus, every %%agent|agent%%-%%principal|principal%% pair in the relation `is acting on behalf of` MUST also appear as an %%employee|employee%%-%%employer|employer%% pair in the relation `works for`.

### Potential representation of parties

The second step of %%onboarding|onboarding%%, i.e. ensuring that an %%actor|actor%% will have effective duties to actually execute %%actions|action%% on behalf of a %%party|party%% can be quite easy, but also very complex.

#### Owning actors

The easiest way to do so (for a %%party|party%%) is to %%own|ownership%% the %%actor|actor%%. %%Human|human-being%% %%actors|actor%%, that are also %%parties|party%%, are postulated to %%own|owner%% themselves, as they are typically endowed with, and exercize their legal, rightful or natural title to control themselves.[^1] Non-human %%actors|actor%%, such as robots, animals, apps on mobile phones, etc. can also be %%owned|owned%%, depending on the %%legal system|legal-system%% of the %%jurisdiction|jurisdiction%% that %%governs|governance%% such %%ownership|ownership%%.

%%Owning|ownership%% an %%actor|actor%%, i.e. (entirely) controlling it, enables the %%owner|owner%% to direct the %%actor|actor%% to execute %%actions|action%%, which the %%actor|actor%% can then exeucte (within the limitations of its capabilities) as that %%party's|party%% %%agent|agent%%. Of course, this still requires the third step of %%onboarding|onboarding%% to be performed, i.e. that the %%owner|owner%% provides the necessary circumstances and conditions for the %%actor|actor%% to do its job.

#### Hiring/Leasing/... actors

A %%party|party%% that wants to use an %%actor|actor%% that it does not %%own|owner%%, must find other ways to ensure that the %%actor|actor%% will have effective duties to actually execute %%actions|action%% on its behalf. This basically boils down to making some kind of agreement with a %%party|party%% that controls such an %%actor|actor%% to an appropriate extent. This can take many forms.

A first form is to reach an agreement with the %%owner|owner%% of the %%actor|actor%%. A well-known form is that where the %%party|party%% hires an %%individual|human-being%%, by establishing and signing a labor contract.
